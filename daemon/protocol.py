...
def read_request(conn):
    ...
    if magic != b"MGV2" or not 0 < n <= MAX_JSON:
        raise ProtocolError("Invalid MGV2 header")
    try:
    ...
    cmd = header.get("cmd")
    if cmd not in ("infer", "info", "shutdown"):
...
def pack_reply(status, metadata, w=0, h=0, channels=0, payload=b""):
    ...
    return (
        REPLY.pack(b"MGN2", status, w, h, channels, len(body))
        + body
...
def request(header, payload=b"", port=PORT, timeout=600):
    ...
    with socket.create_connection(("127.0.0.1", port), timeout=timeout) as conn:
        ...
        if magic != b"MGN2" or n > MAX_JSON:
            raise ProtocolError("Invalid MGN2 reply")
