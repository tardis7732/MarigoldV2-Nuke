// Marigold V2 Normals OFX frontend, derived from MoGe-nuke.
// Copyright (c) 2026 Sumit Chatterjee. MIT; see licenses/MoGe-nuke-MIT.txt.
// Modified: normals-only protocol, bounded replies, cache invalidation, hidden
// launcher, seed/resolution and explicit axis controls. No model in the host.
...
// this order:
//   1. MARIGOLD_NUKE_ROOT environment variable
//   2. marigoldv2.cfg next to the .ofx (written by install.ps1): key=value lines
...
static void logf(const char *fmt, ...) {
    ...
    va_end(ap);
    fprintf(stderr, "[MarigoldV2Normals OFX] %s\n", buf);
    fflush(stderr);
}
...
struct Instance {
    ...
    std::vector<float> cachePlanes; // XYZ + alpha, top row first
};
...
static void showError(Instance *inst, const std::string &msg) {
    ...
    if (gMessage && inst && inst->effect)
        gMessage->message(inst->effect, kOfxMessageError, nullptr, "MarigoldV2Normals: %s", msg.c_str());
}
...
static Params readParams(Instance *in, double t) {
    ...
    p.flipX = getIntParam(in->pFlipX, t);
    p.flipY = getIntParam(in->pFlipY, t);
    p.flipZ = getIntParam(in->pFlipZ, t);
...
static bool spawnDaemon(const Params &p, proc_t &proc, std::string &err) {
    ...
    if (!fileExists(p.python)) {
        err = "python not found: '" + p.python + "'. Run the installer from the MarigoldV2-Nuke repo, "
...
static bool ensureConnected(Instance *in, const Params &p, std::string &err, bool &cancelled) {
    ...
    if (!p.autoStart) {
        err = "no Marigold V2 daemon on port " + std::to_string(p.port) +
    ...
    if (gProgress) gProgress->progressStart(in->effect, "MarigoldV2Normals: starting daemon and loading the model ...");
...
static bool daemonInfer(Instance *in, const Params &p, int w, int h,
    ...
    for (int attempt = 0; attempt < 2; ++attempt) {
        ...
        bool ok = sendAll(in->sock, "MGV2", 4) && sendAll(in->sock, &jlen, 4) &&
...
static void loadConfig() {
    ...
    // 2. marigoldv2.cfg beside the .ofx
    std::string cfgPath = pluginDir() + "/marigoldv2.cfg";
    ...
    std::string envRoot = envVar("MARIGOLD_NUKE_ROOT");
    ...
    if (gRoot.empty())
        logf("no MARIGOLD_NUKE_ROOT and no marigoldv2.cfg -- fill in the Setup tab on the node");
    else
...
static OfxStatus describeInContext(OfxImageEffectHandle effect) {
    ...
    gProp->propSetInt(p, kOfxParamPropDefault, 0, 512);
    gProp->propSetInt(p, kOfxParamPropMin, 0, 0);
    gProp->propSetInt(p, kOfxParamPropMax, 0, 2048);
    gProp->propSetInt(p, kOfxParamPropDisplayMax, 0, 1024);
    p = defineParam(set, kOfxParamTypeInteger, "seed", "seed", "VAE sampling seed, reset per frame.", "Normals");
    gProp->propSetInt(p, kOfxParamPropDefault, 0, 2025);
    gProp->propSetInt(p, kOfxParamPropMin, 0, 0);
    ...
    p = defineParam(set, kOfxParamTypeChoice, "inputColorspace", "input encoding",
        ...
        "Linear sRGB applies only the sRGB transfer function; it is not an ACES transform.", "Normals");
    ...
    for (const char *name : {"flipX", "flipY", "flipZ"}) {
        p = defineParam(set, kOfxParamTypeBoolean, name, name,
            "Negate this normal component. Default preserves the official Marigold camera-space axes.", "Normals");
        gProp->propSetInt(p, kOfxParamPropDefault, 0, 0);
    }
    p = defineParam(set, kOfxParamTypeInteger, "cacheRevision", "cache revision",
        "Increment to force inference after replacing model assets or restarting the engine.", "Normals");
    gProp->propSetInt(p, kOfxParamPropDefault, 0, 0);
    gProp->propSetInt(p, kOfxParamPropMin, 0, 0);
    gProp->propSetInt(p, kOfxParamPropMax, 0, 2147483647);
...
static OfxStatus render(OfxImageEffectHandle effect, OfxPropertySetHandle inArgs) {
    ...
    if (!hit) {
        ...
        if (!daemonInfer(in, p, W, H, rgb, planes, err, cancelled)) {
            gEffect->clipReleaseImage(src.handle);
